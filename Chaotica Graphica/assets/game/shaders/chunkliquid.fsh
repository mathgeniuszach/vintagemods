...
void main() 
{
	...
	bool isLava = (waterFlags & 0x2000000) > 0;
	
	if (isLava) wfc = waterFlowCounter / 10.0 * (1 + 5 * vn);
	else wfc = waterFlowCounter / 1.8 * (1 + 5 * vn);
	...
	float k = gl_FragCoord.x + gl_FragCoord.y + gl_FragCoord.z + sunPosRel.x + sunPosRel.y + sunPosRel.z;
	float a = k + gl_FragCoord.x + texColor.r;
	float b = k + gl_FragCoord.y + texColor.g;
	float c = k + gl_FragCoord.z + texColor.b;
	drawPixel(vec4(rand(c), rand(b), rand(a), rand(k)), accuWeight);
}
